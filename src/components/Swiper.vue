<template>
  <div class="swiper" :style="setCarouselSize()">
    <div class="window">
      ...
      <ol class="point-wrap">
        <li :class="{active:i==currentIndex}" v-for="(e,i) in data" :key="i"></li>
      </ol>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    setCarouselSize() {
      return "height: " + (288 / 714) * document.body.clientWidth + "px";
    },
    // 窗口变化 重新初始化
    windowChange() {
      const that = this;
    ...
    next() {
      ...
      let left = center - 1 < 0 ? this.data.length - 1 : center - 1;
      // 右边的 如果超过了最大图片数量 取第一张图片下标
      let right = center + 1 == this.data.length ? 0 : center + 1;
